...
/**
 * The {@code DFADeterminisation} class is responsible for converting a regular
 * expression
 * syntax tree (represented by {@code RegExTree}) into a Non-Deterministic
 * Finite Automaton (NDFA).
 *
 * <p>
 * This class processes the structure of the regular expression tree, handling
 * basic regex operations
 * such as concatenation, alternation, and Kleene star, and converts them into
 * an NDFA. It also supports
 * individual characters or wildcard expressions (DOT).
 * </p>
 *
 * <p>
 * The generated NDFA is composed of states and transitions, and it follows the
 * formal rules of
 * non-deterministic finite automata (NDFA), with epsilon (ε) transitions when
 * necessary.
 * </p>
 */
public class NDFAParser {
    ...
    /**
     * Converts a regular expression syntax tree into a Non-Deterministic Finite
     * Automaton (NDFA).
     *
     * <p>
     * This method processes the tree structure of the regular expression and builds
     * the corresponding NDFA.
     * It recursively processes the nodes in the tree and handles the following
     * cases:
     * <ul>
     ...
     * </p>
     *
     * @param arbreRegEx The regular expression syntax tree to be converted into an
     *                   NDFA.
     * @return An {@code DFA} representing the generated NDFA from the regular
     *         expression tree.
     */
    public static NDFA parseTreeToNDFA(RegExTree arbreRegEx) {
        ...
        if (arbreRegEx.getRoot() == RegExTreeParser.CONCAT) {
            ...
            gauche.etatAcceptant.ajouterTransition(droite.etatInitial); // Connect left accept state to right start
                                                                        // state
        ...
        // Fallback case: If the expression is unknown or unsupported, return a default
        // NDFA
